...
class Cub2011(Dataset):
    def __init__(self, root = "./data", train = True, transform = None, download = False):
        # Hardcoded variables
        if train:
            self.base_folder = 'CUB_200_2011/dataset/train_corners'
        else:
            self.base_folder = 'CUB_200_2011/dataset/test_full'
        ...
        if self.train:
            self.images = torchvision.datasets.ImageFolder(os.path.join(self.root, self.base_folder), transform=transform)
    ...
    def __len__(self):
        return len(self.data) if not self.train else len(self.images)

    def __getitem__(self, idx):
        if self.train:
            img, target = self.images.__getitem__(idx)
