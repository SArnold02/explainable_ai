...
def upsample(
    ...
    # Create directory
    dir = os.path.join(os.path.join(args.output_path, "images"), "upsample")
    os.makedirs(dir, exist_ok=True)

    with torch.no_grad():
        sim_maps, project_info = get_similarity_maps(tree, project_info, log)
        log.info("Upsampling prototypes for visualization...")
        ...
        for node, j in tree._out_map.items():
            if node in tree.branches():
                ...
                plt.imsave(
                    fname=os.path.join(dir,'%s_heatmap_latent_similaritymap.png'%str(decision_node_idx)),
                ...
                plt.imsave(
                    fname=os.path.join(dir,'%s_heatmap_original_image.png'%str(decision_node_idx)),
                ...
                assert W == H
                masked_similarity_map[prototype_index // W, prototype_index % W] = 1
                
                upsampled_prototype_pattern = cv2.resize(masked_similarity_map,
                                                    dsize=(img_size[1], img_size[0]),
                                                    interpolation=cv2.INTER_CUBIC)
                plt.imsave(
                    fname=os.path.join(dir,'%s_masked_upsampled_heatmap.png'%str(decision_node_idx)),
                ...
                plt.imsave(
                    fname=os.path.join(dir,'%s_nearest_patch_of_image.png'%str(decision_node_idx)),
                    arr=high_act_patch,
                    vmin=0.0,
                    vmax=1.0
                )
...
def get_similarity_maps(tree: ProtoTree, project_info: dict, log: Logger = None):
    log.info("Calculating similarity maps (after projection)...")
